...
void main()
{
	...
	PBYTE p;
	HANDLE hUsbBusDev = FindFirstUsbBusDev(); //‰ç›qUSB•ˆ‰WEWP•ÔŽšE
	if (hUsbBusDev) //”å›ŠäšØUSB•ˆ‰W
	{
		WCHAR szDevPath[MAX_PATH] = {};
		FindUsbBusGetDevPath(hUsbBusDev,szDevPath,ARRAYSIZE(szDevPath)); //”Ÿ‹b‹y™¹ŒôUSB•ˆ‰W‘µ´EhNext‰ç›q˜aŒô
		FindUsbBusClose(hUsbBusDev); //w‰t‰ç›q
		if (wcslen(szDevPath) > 0)
		{
			HANDLE h = InitWinPhoneProjectionClient(szDevPath); //Š••ºŽN—UšOEWP•ÔŽšŽŠ–E—G›¨E
			if (h)
			{
				_FOREVER_LOOP{
					if (!ReadWinPhoneScreenImageAsync(h)) //ŒA–l‹¿”‹”“
						break;
					UINT nWidth = 0,nHeight = 0,nOrientation = WP_SCR_ORI_DEFAULT;
					DWORD dwImageBits = 0;
					DWORD dwSrtride = 0;
					if (!WaitWinPhoneScreenImageComplete(h,&p,&nWidth,&nHeight,&dwImageBits,&dwSrtride,&nOrientation)) //‹gŠû•ûÛ‹¿”Ÿ—ŠŠh
						break;
				...
				FreeWinPhoneProjectionClient(h); //•ËŒd—UšOlŽF‹É
			}
		}
	}
	pWICFactory->Release();
